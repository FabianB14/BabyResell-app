...
const Register = () => {
  ...
  const { register, loading, error, isAuthenticated, testConnection, login } =
    useAuth(); // Added 'login' here
    ...
    const checkApiAccess = async () => {
      try {
        ...
        setApiStatus({
          checked: true,
          working: true,
          message: `API connected successfully: ${JSON.stringify(
            response.data
          )}`,
        });
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.token) {
        ...
        // If user data is included, store it and use login function
        if (response.data.user) {
          localStorage.setItem("user", JSON.stringify(response.data.user));
          
          // Use the login function from auth context for auto-login
          if (login) {
  ...
  const apiStatusStyle = {
    ...
    fontSize: "14px",
    textAlign: "center",
    backgroundColor: apiStatus.working
      ? "rgba(5, 150, 105, 0.1)"
      : "rgba(220, 38, 38, 0.1)",
  ...
  return (
    <div style={containerStyle}>
      <div style={formContainerStyle}>
        ...
        {(error || localError) && (
          <div style={errorStyle}>{error || localError}</div>
        )}
